...
func main() {
	...
	k := "fleming_ian"
	v := []string{"steaks", "cigars", "espionage"}

	printMap(addToMap(mp, k, v))

	dk := "no_dr"
	printMap(deleteFromMap(mp, dk))

	tgg := []string{"in", "my", "younger", "and", "more", "vulnerable", "years", "my", "father", "gave", "me", "some",
...
func addToMap(mp map[string][]string, k string, v []string) map[string][]string {
	mp[k] = v
	return mp
}

func deleteFromMap(mp map[string][]string, dk string) map[string][]string {
	delete(mp, dk)
	return mp
}

func countWords(w []string) map[string]int {
